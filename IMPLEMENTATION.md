...
- `services/backend/shopify-app` — Shopify integration layer (OAuth, webhooks, storefront scripts). Requires `SHOPIFY_API_KEY` etc.
  ...
  - Standalone server: `services/backend/shopify-app/src/server.js`
  - Legacy monolithic app (for backward compatibility): `services/backend/shopify-app/src/app.js`

- `services/backend/api` — Business & domain API server (dashboard, creator/brand endpoints, price history). Boots WITHOUT Shopify credentials.
  - Entry point: `services/backend/api/src/createApp.js`
  - Standalone server: `services/backend/api/src/server.js`

- `services/backend/redirect` — Link routing & redirect server (link creation, supporter routing, click tracking). Boots WITHOUT Shopify credentials.
  - Entry point: `services/backend/redirect/src/createApp.js`
  - Standalone server: `services/backend/redirect/src/server.js`
...
- Shared source tree for domain logic, data access, and HTTP utilities:
  - Source of truth: `services/backend/shopify-app/src/` (shared across all services)
...
|---------|------|---------------------|
| `services/backend/api` | Dashboard routes, debug routes, price history | No |
| `services/backend/redirect` | Link creation, supporter routing, redirect handling | No |
| `services/backend/shopify-app` | OAuth, webhooks, storefront scripts | Yes |
...
- SQLite remains a backend-local fallback provider for the backend.
  - Implemented in: `services/backend/shopify-app/src/db/database.js`
  - Implemented in: `services/backend/shopify-app/src/repositories/*.js`
...
- `services/backend/api` and `services/backend/redirect` now have live implementations via their `createApp.js` entrypoints, sharing the source tree under `services/backend/shopify-app/src/`.
  - Runtime authority: `services/backend/api/src/createApp.js`
  - Runtime authority: `services/backend/redirect/src/createApp.js`
...
- **Shopify** (`shopify-env.js`): `SHOPIFY_API_KEY`, `SHOPIFY_API_SECRET`, `SHOPIFY_SCOPES`, `SHOPIFY_APP_URL`, `SHOPIFY_CALLBACK_URL`, `SHOPIFY_API_VERSION` — required ONLY by `services/backend/shopify-app` and `services/backend`

`services/backend/api` and `services/backend/redirect` boot without Shopify credentials.
...
Legacy monolithic app for backward compat: `services/backend/shopify-app/src/app.js`
...
Implement in: `services/backend/shopify-app/src/config/shared-env.js`
Implemented in: `services/backend/shopify-app/src/repositories/shop-repository.js`
Implemented in: `services/backend/shopify-app/src/repositories/link-click-repository.js`
Implemented in: `services/backend/shopify-app/src/repositories/order-attribution-repository.js`

### Proven active runtime paths

- Shopify attribution flow has been exercised end to end through `services/backend/shopify-app`.
  - Implemented in: `services/backend/shopify-app/src/services/link-service.js`
  - Implemented in: `services/backend/shopify-app/src/services/storefront-script-service.js`
...
- Dev bypass remains active for backend auth guards outside production.
  - Implemented in: `services/backend/shopify-app/src/middleware/auth-context.js`
...
- `GET /auth`
  ...
  - Implemented in: `services/backend/shopify-app/src/routes/auth-routes.js`
  - Implemented in: `services/backend/shopify-app/src/controllers/auth-controller.js`
  - Implemented in: `services/backend/shopify-app/src/services/oauth-service.js`
...
- `GET /r/:creator_id/:link_id`
  ...
  - Implemented in: `services/backend/shopify-app/src/controllers/link-controller.js`
  - Implemented in: `services/backend/shopify-app/src/services/link-service.js`
...
- `POST /links/create`
  ...
  - Implemented in: `services/backend/shopify-app/src/routes/link-routes.js`
  - Implemented in: `services/backend/shopify-app/src/controllers/link-controller.js`
  - Implemented in: `services/backend/shopify-app/src/services/link-service.js`
...
- `GET /storefront/atribe.js`
  ...
  - Implemented in: `services/backend/shopify-app/src/routes/storefront-routes.js`
  - Implemented in: `services/backend/shopify-app/src/controllers/storefront-script-controller.js`
  - Implemented in: `services/backend/shopify-app/src/services/storefront-script-service.js`
  - Used by: `services/backend/shopify-app/extensions/atribe-theme-extension/assets/atribe-app-embed.js`

- Theme app embed
  - Preferred storefront injection path for Shopify attribution persistence.
  - Implemented in: `services/backend/shopify-app/extensions/atribe-theme-extension/blocks/atribe-app-embed.liquid`
  - Implemented in: `services/backend/shopify-app/extensions/atribe-theme-extension/assets/atribe-app-embed.js`
...
- `POST /webhooks/orders_paid`
  ...
  - Implemented in: `services/backend/shopify-app/src/routes/webhook-routes.js`
  - Implemented in: `services/backend/shopify-app/src/controllers/webhook-controller.js`
  - Implemented in: `services/backend/shopify-app/src/services/order-attribution-service.js`
...
- `POST /webhooks/app_uninstalled`
  ...
  - Implemented in: `services/backend/shopify-app/src/controllers/webhook-controller.js`
  - Writes/deletes through: `services/backend/shopify-app/src/repositories/shop-repository.js`
...
- `GET /price-history/lookup?url=...` and `GET /api/price-history/lookup?url=...`
  ...
  - Implemented in: `services/backend/shopify-app/src/routes/price-history-routes.js`
  - Implemented in: `services/backend/shopify-app/src/controllers/price-history-controller.js`
  - Implemented in: `services/backend/shopify-app/src/services/priceHistory/index.js` (orchestrator)
  - Implemented in: `services/backend/shopify-app/src/services/priceHistory/productInfo.js` (URL parsing)
  - Implemented in: `services/backend/shopify-app/src/services/priceHistory/productHistoryProvider.js` (primary)
  - Implemented in: `services/backend/shopify-app/src/services/priceHistory/priceHistoryAppProvider.js` (fallback)
  - Implemented in: `services/backend/shopify-app/src/services/priceHistory/htmlParsers.js` (HTML parsing)
  - Implemented in: `services/backend/shopify-app/src/services/priceHistory/confidence.js` (candidate scoring)
  - Implemented in: `services/backend/shopify-app/src/services/priceHistory/cache.js` (in-memory cache)
...
- `GET /creator/links?creator_id=...`
  ...
  - Implemented in: `services/backend/shopify-app/src/controllers/dashboard-controller.js`
  - Implemented in: `services/backend/shopify-app/src/services/dashboard-service.js`
  - Reads: `shopify_links`
...
Implemented in: `services/backend/shopify-app/src/app.js`
Implemented in: `services/backend/shopify-app/src/routes/debug-routes.js`
...
- `profiles`
  ...
  - Implemented in: `services/backend/shopify-app/src/repositories/user-repository.js`
...
- `creator_profiles`
  ...
  - Implemented in: `services/backend/shopify-app/src/repositories/creator-repository.js`
...
- `tribe_memberships`
  ...
  - Implemented in: `services/backend/shopify-app/src/repositories/user-creator-weight-repository.js`
  - Written directly by mobile UI in current app flow: `apps/client/src/context/AppContext.js`
...
- `shopify_brand_integrations`
  ...
  - Implemented in: `services/backend/shopify-app/src/repositories/brand-integration-repository.js`
  - Updated by install persistence in: `services/backend/shopify-app/src/repositories/shop-repository.js`
...
- `shopify_click_weight_snapshots`
  ...
  - Implemented in: `services/backend/shopify-app/src/repositories/click-weight-snapshot-repository.js`

- `shopify_orders`
  - Stored Shopify orders received through backend webhooks.
  - Implemented in: `services/backend/shopify-app/src/repositories/shopify-order-repository.js`
...
- `shopify_order_commissions`
  ...
  - Implemented in: `services/backend/shopify-app/src/repositories/order-commission-repository.js`
...
- `shopify_campaigns`
  ...
  - Implemented in: `services/backend/shopify-app/src/repositories/campaign-repository.js`
...
### 1. `services/backend/shopify-app/README.md` can lag runtime behavior if not maintained with backend changes

- Current behavior:
  - service-local README is secondary documentation, not runtime authority.
  - README truth: `services/backend/shopify-app/README.md`
...
### 2. Root README layout still overstates `services/backend/api` and `services/backend/redirect` as separate codebases

- Current behavior:
  - root README presents `services/backend/api` and `services/backend/redirect` as core backend pieces with their own implementations.
  - `services/backend/api` and `services/backend/redirect` now have live `createApp.js` entrypoints sharing the source tree under `services/backend/shopify-app/src/`. They boot independently without Shopify credentials.
  - README truth: `README.md`

- Conflicting runtime truth:
  - the actual implemented code lives in a shared source tree under `services/backend/shopify-app/src/`, composed through `services/backend/src/createApp.js`.
  - Implemented in: `services/backend/src/createApp.js`
...
- Mobile Shopify creator-brand connection through backend endpoints.
  ...
  - Implemented in: `apps/client/src/screens/AddAffiliateLinksScreen.js`
  - Implemented in: `apps/client/src/screens/BrandProgramWebViewScreen.js`
  - Used by backend: `services/backend/shopify-app/src/routes/dashboard-routes.js`
...
- Price history lookup for Amazon and Flipkart product links via ProductHistory.in (primary) and PriceHistoryApp.com (fallback).
  ...
  - Implemented in: `services/backend/shopify-app/src/services/priceHistory/index.js`
