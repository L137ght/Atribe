-- Re-assert support/reward/share feature tables so PostgREST can see them,
-- then explicitly request a schema reload.
...
alter table public.share_links
  ...
  add column if not exists owner_user_agent_hash text,
  add column if not exists owner_fingerprint_hash text;
